...
extern "C" void AboutReport(rapidjson::Value&                   request,
    ...
    response.AddMember(
        "description",
        Value().SetString(
            "Shows platform operation statistics for a specified day, including the number "
            "of regular and error messages in the journal, critical errors, and connection logs.",
            allocator),
        allocator);
    response.AddMember("type", static_cast<int>(ReportType::Daily), allocator);
    response.AddMember("key", Value().SetString("DAILY_LOGS_REPORT", allocator), allocator);
}
...
extern "C" void CreateReport(rapidjson::Value&                   request,
    ...
    for (size_t i = 0; i < line_keys.size(); ++i) {
        ...
        line_nodes.push_back(
            Line({}, props({{"type", "monotone"}, {"dataKey", line_keys[i]}, {"stroke", color}})));
    }

    Node server_logs_chart_node =
